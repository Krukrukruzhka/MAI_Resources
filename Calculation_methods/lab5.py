...
def explicit(K, t, tau, h, a, b, c, x, approx):
    ...
    for k in range(K - 1):
        ...
        for j in range(1, N - 1):
            U[k + 1, j] = tau * (a * (U[k, j - 1] - 2 * U[k, j] + U[k, j + 1]) / h**2 + b * (U[k, j + 1] - U[k, j - 1]) / (2 * h) + c * U[k, j]) + U[k, j]
        if approx == 1:
            ...
            U[k + 1, 0] = (func_border1(a, b, c, t) * h * tau * (2 - b * h) - U[k + 1, 1] * (2 * tau) - U[k, 0] * h**2) / (-2 * tau - h**2 + c * tau * h**2 + h * tau * (2 - b * h))
            U[k + 1, N - 1] = (func_border2(a, b, c, t) * h * tau * (2 + b * h) + U[k + 1, N - 2] * (2 * tau) + U[k, N - 1] * h**2) / (2 * tau + h**2 - c * tau * h**2 + h * tau * (2 + b * h))
    return U
