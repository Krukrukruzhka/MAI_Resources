...
# iteration = 0
#
# top_center = np.array([x0, y0+height/2, z0])
# bottom_center = np.array([x0, y0-height/2, z0])
#
# top_coordinates = np.array([[x0 + r1 * cos(radians(i*angle_step)), y0+height/2, z0 + r1 * sin(radians(i*angle_step))] for i in range(sides+1)])
# bottom_coordinates = np.array([[x0 + r2 * cos(radians(i*angle_step)), y0-height/2, z0 + r2 * sin(radians(i*angle_step))] for i in range(sides+1)])
#
#
# def move(dx: float = 0, dy: float = 0, dz: float = 0):
#     global top_center, top_coordinates, bottom_coordinates, bottom_center, iteration
#
#     top_coordinates[:, 0] += dx
#     bottom_coordinates[:, 0] += dx
#     top_center[0] += dx
#     bottom_center[0] += dx
#
#     top_coordinates[:, 1] += dy
#     bottom_coordinates[:, 1] += dy
#     top_center[1] += dy
#     bottom_center[1] += dy
#
#     top_coordinates[:, 2] += dz
#     bottom_coordinates[:, 2] += dz
#     top_center[2] += dz
#     bottom_center[2] += dz
#
#
...
#     global top_center, top_coordinates, bottom_coordinates, bottom_center, iteration
#
#     iteration = (iteration + 1) % 180
#     speed: float = 0.2
#     dx = (top_center[0]+bottom_center[0])/2
#     dx *= cos(radians(iteration*speed))-1
#     if abs(dx) < pow(10, -6):
#         iteration = 0
#
#     top_coordinates[:, 0] += dx
...
#
#
# def rotate_cordinates(x: float = 360, y: float = 360, z: float = 360):
#     global top_coordinates, bottom_coordinates, top_center, bottom_center
#
#     top_center = np.dot(top_center, rotate_matrix(x, y, z))
#     bottom_center = np.dot(bottom_center, rotate_matrix(x, y, z))
#     top_coordinates = np.dot(top_coordinates, rotate_matrix(x, y, z))
#     bottom_coordinates = np.dot(bottom_coordinates, rotate_matrix(x, y, z))
#
#
# def scale(value: float):
#     global top_coordinates, bottom_coordinates, top_center, bottom_center
#
#     top_coordinates *= value
#     bottom_center *= value
#     bottom_coordinates *= value
#     top_center *= value
#
#
# def draw_figure():
...
#         glVertex3f(top_coordinates[i][0], top_coordinates[i][1], top_coordinates[i][2])
#         glVertex3f(bottom_coordinates[i][0], bottom_coordinates[i][1], bottom_coordinates[i][2])
#     glEnd()
#
#     glBegin(GL_TRIANGLE_FAN)
#     glVertex3f(*top_center)
#     for i in range(sides+1):
#         glVertex3f(top_coordinates[i][0], top_coordinates[i][1], top_coordinates[i][2])
#     glEnd()
#
#     glBegin(GL_TRIANGLE_FAN)
#     glVertex3f(*bottom_center)
#     for i in range(sides+1):
#         glVertex3f(bottom_coordinates[i][0], bottom_coordinates[i][1], bottom_coordinates[i][2])
#     glEnd()
#
...
#
#     glLightfv(GL_LIGHT0, GL_POSITION, [0, 1.5, 0.6, 1.0])
...
#
#         rotate_cordinates(alpha, betta, gamma)
#
#         animation()
#         draw_figure()
#
...
class Taper:
    ...
    def animation(self):
        self.iteration = (self.iteration + 1) % 180
        speed: float = 0.2
        dx = (self.top_center[0] + self.bottom_center[0]) / 2
        dx *= cos(radians(self.iteration * speed)) - 1
        if abs(dx) < pow(10, -6):
            self.iteration = 0

        self.top_coordinates[:, 0] += dx
        self.bottom_coordinates[:, 0] += dx
        self.top_center[0] += dx
        self.bottom_center[0] += dx
...
def main():
    ...
    glMaterial(GL_FRONT, GL_DIFFUSE, [1.0, 1.0, 0.0, 1.0])
    ...
    while True:
        ...
        print((taper.top_center[0]+taper.bottom_center[0])/2)
